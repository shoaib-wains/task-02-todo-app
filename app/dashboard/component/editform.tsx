...
export default function EditForm({id,todo}:any){
    ...
    const [newtodo,setNewtodo]=useState(todo)
     const handleSubmit= async(e:any)=>{
         ...
         try{ 
          const res= await fetch(`http://localhost:3000/dashboard/apitodos/${id}`, {method:'PUT',
        ...
        <div className={`${styles.list} pb-10 bg-transparent justify-center`}>
          ...
          />
            ...
            {/* className={`${styles.input} w-112 h-14 rounded-full border-4`} */}
              <input onChange={e=>{setNewtodo(e.target.value)}} name='list' value={newtodo}
               className={clsx({
                    [styles.input]: theme === '',
